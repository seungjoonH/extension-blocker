...
확장자 정책 열에서는 업로드 최대 크기를 두고, 그 아래 카드 안에 "확장자" 제목과 설명, 커스텀 입력 → 고정 확장자 → 커스텀 목록을 둔다. 데스크톱에서는 확장자 정책과 파일 업로드의 섹션 제목과 설명을 카드 밖 동일 레벨에 둔다.
...
- 구성 요소: 입력 필드(`maxlength=20`), 글자 수 표시(`n/20`), 추가 버튼, 등록 개수 표시(`n/200`), 등록된 확장자 목록(각 항목에 삭제 버튼). 입력부와 목록부는 분리된 컴포넌트이며 상태는 상위 훅에서 공유한다.
- 행동: 입력, 추가(버튼 클릭 또는 Enter 제출), 삭제
- 상태 변화
  ...
  - 추가 클릭 또는 Enter 제출 시 버튼이 "추가 중..." 상태로 바뀌며 비활성화되고, 성공하면 목록에 반영되며 입력값이 초기화되고 자동 소멸 토스트로 성공을 안내한다.
