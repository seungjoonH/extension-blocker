...
단일 화면 **"확장자 차단 및 업로드 관리"** 로 구성한다. 화면 안 영역은 다음 순서로 배치한다.

| 배치 | 영역 | 목적 |
|---|---|---|
| 왼쪽(좁은 화면에서는 위) | 확장자 정책 | 업로드 최대 크기, 커스텀/고정 확장자 차단 설정을 한 열에서 관리한다 |
| 오른쪽(좁은 화면에서는 아래) | 파일 업로드 섹션 | 실제 파일을 업로드하고 정책 적용 결과를 확인한다 |
| — | 전역 알림 영역 | 토스트 메시지를 표시한다(화면 전역에 걸쳐 하나의 위치) |

확장자 정책 열에서는 업로드 최대 크기를 두고, 그 아래 카드 안에 "확장자" 제목과 설명, 단일/일괄 모드 토글 → 가져오기/내보내기/초기화 → 입력(단일 또는 일괄) → 고정 확장자 → 커스텀 목록을 둔다. 데스크톱에서는 확장자 정책과 파일 업로드의 섹션 제목과 설명을 카드 밖 동일 레벨에 둔다.
...
- 구성 요소: 단일/일괄 세그먼트 컨트롤, (단일 모드) 입력 필드(`maxlength=20`), 글자 수 표시(`n/20`), 추가 버튼, (일괄 모드) 쉼표 구분 텍스트 영역과 일괄 등록 버튼, `extignore.txt` 가져오기와 내보내기 버튼, 등록 개수 표시(`n/200`), 등록된 확장자 목록(각 항목에 삭제 버튼), 정책 초기화 버튼. 입력부와 목록부는 분리된 컴포넌트이며 상태는 상위 훅에서 공유한다.
- 행동: 모드 전환, 입력, 추가(버튼 클릭 또는 Enter 제출), 일괄 등록, `extignore.txt` 가져오기와 내보내기, 삭제, 정책 초기화
- 상태 변화
  - 입력값이 정규화 후 빈 문자열이면 추가 버튼이 비활성화된다. 버튼 비활성화 상태만으로 의미를 전달하지 않고, 입력 필드의 레이블이나 안내 문구로 입력이 필요하다는 점을 함께 제공한다.
  - Enter 키 제출도 추가 버튼과 동일한 `canSubmit` 조건(빈 값, 제출 중, 200개 초과, 형식 오류)을 적용한다.
  - 추가 클릭 또는 Enter 제출 시 버튼은 유휴 라벨 너비를 유지한 채 원형 스피너로 진행 상태를 표시하며 비활성화되고, 성공하면 목록에 반영되며 입력값이 초기화되고 자동 소멸 토스트로 성공을 안내한다.
  ...
  - **모드 전환**: 입력 방식을 단일/일괄로 전환하는 세그먼트 컨트롤을 제공한다. 전환 시 확인 절차 없이 두 모드의 입력값을 모두 초기화한다.
  - **일괄 입력**: 쉼표로 구분한 텍스트를 입력받는다(줄바꿈은 구분자로 인정하지 않는다). 분리, trim, 빈 값 제거, 항목별 정규화, 형식 검증(항목별 20자 제한 포함), 입력 내부 중복 제거는 클라이언트에서 즉시 수행한다. 형식 오류가 하나라도 있으면 API 호출 없이 전체 제출을 막고, 문제 항목 전체를 사유 구분 없는 하나의 목록으로 인라인 안내한다(예: "올바르지 않은 항목이 있습니다: my-ext, very-long-name"). 형식 검증을 통과해 서버에 제출한 뒤에는 개별 항목의 처리 결과(신규 등록, 자동 활성화, 기존 커스텀 중복 제외)를 항목별로 강조하거나 포커스를 이동하지 않고, "n개 등록됨, {목록} 활성화됨, 이미 등록된 n개 제외"처럼 하나의 요약 문장을 자동 소멸 성공 토스트로만 안내한다. 여러 고정 확장자가 동시에 자동 활성화되어도 단일 모드의 "해당 항목으로 포커스 이동" 원칙은 일괄 처리에는 적용하지 않는다. 200개 초과처럼 사용자가 입력을 줄여야 하는 서버 검증 실패는 닫기 가능한 토스트가 아니라 확장자 카드의 인라인 오류로 안내한다. 일괄 등록 버튼도 진행 중에는 라벨 너비를 유지한 채 원형 스피너로 표시한다.
  - **`extignore.txt` 가져오기**: 파일을 선택하는 즉시 내용을 줄 단위로 분리해 파싱과 검증을 수행하고 자동으로 제출한다. 일괄 입력 텍스트 영역에 내용을 채워 사용자가 확인하거나 수정하는 중간 단계는 두지 않는다. 형식 오류와 한도 초과는 일괄 입력과 동일한 인라인 방식으로 안내하며, 가져오기는 모드와 무관하게 동작하므로 오류도 단일/일괄 모드와 관계없이 확장자 카드에 표시한다. 가져오기 진행 중에는 버튼에 원형 스피너를 표시한다.
  - **`extignore.txt` 내보내기**: 서버 요청 없이 화면이 이미 불러온 정책 상태로 파일 내용을 즉시 생성해 다운로드한다.
  - **정책 초기화**: 모드 토글 아래 가져오기/내보내기 오른쪽에 초기화 버튼을 둔다. 클릭 시 네이티브 `confirm()` 대화상자로 확인을 받고, 취소하면 아무 요청도 보내지 않는다. 확인하면 커스텀 확장자 전체 삭제와 고정 확장자 전체 비활성화를 요청하고, 성공하면 전체 정책을 다시 조회해 화면을 갱신하며 자동 소멸 성공 토스트로 안내한다. 업로드 최대 크기 정책은 초기화 대상이 아니므로 화면에도 변화가 없다. 실패하면 화면 상태를 바꾸지 않고 닫기 가능한 토스트로 오류를 안내한다. 고정 확장자 저장(Debounce 대기 포함), 커스텀 확장자 추가, 삭제, 일괄 등록 등 다른 영역이 하나라도 진행 중이면 초기화 버튼을 비활성화해, 초기화와 다른 저장 작업이 겹쳐 화면 상태가 혼란스러워지는 상황을 막는다. 초기화 진행 중에도 버튼 너비를 유지한 채 원형 스피너를 표시한다.
...
- 구성 요소: 파일 선택 입력(클릭 선택, `multiple` 속성을 사용하지 않아 한 번에 파일 하나만 선택 가능), 업로드 실행 버튼, 진행 상태 표시(불확정 스피너와 텍스트), 결과 표시 영역(직전 시도 1건), 그 아래 **업로드된 파일 목록**(원본 파일명, 크기, 업로드 시각, 다운로드, 삭제)
- 행동: 파일 선택, 업로드 실행, 목록 다운로드, 목록 삭제(보호 시드는 삭제 불가)
- 업로드 성공 시 직전 결과 안내와 함께 목록을 다시 불러온다
- 상태 변화는 8절에서 상세히 다룬다. 목록 공개와 보호 시드 정책은 `REQUIREMENTS.md` 원본 파일명 저장 절과 `CONSIDERATIONS.md`를 따른다.

### 5.5 전역 알림 영역

- 토스트는 알림별로 고유한 항목으로 생성되며, 새 토스트가 발생해도 기존 토스트를 대체하지 않고 함께 쌓여 각각 독립적으로 표시된다.
- 다만 동일한 종류(성공/실패)와 동일한 메시지의 토스트가 이미 표시 중이면 중복으로 쌓지 않고 기존 항목을 유지한다. 성공 토스트는 표시 시간이 갱신되고, 실패 토스트는 기존 항목이 그대로 유지된다. 같은 문구라도 성공과 실패는 서로 다른 알림으로 취급해 함께 표시될 수 있다.
...
- 데스크톱(넓은 화면)에서는 확장자 섹션과 파일 업로드 섹션을 좌우 2단으로 배치하고, 좁은 화면에서는 위아래 1단으로 쌓는다. 별도 전용 레이아웃 파일을 두지 않고 그리드 기본 동작으로 처리한다. 고정/커스텀 확장자 목록은 `flex-wrap`으로 자동 줄바꿈된다.
...
| 커스텀 확장자 일괄 입력 모드, `extignore.txt` 가져오기와 내보내기, 확장자 정책 초기화 | 5.2 |
